/**
 * Netherlands RSS Service — Government of the Netherlands (Government.nl)
 *
 * Fetches official Government.nl residence and immigration RSS 2.0 updates
 * and normalises each item into our NewsArticle type.
 *
...
const SOURCE = getSourceById("netherlands-gov")!;

// ─────────────────────────────────────────────────────────────────────────────
// Entry normalizer (Netherlands-specific)
// ─────────────────────────────────────────────────────────────────────────────
...
function normalizeEntry(entry: any): NewsArticle | null {
  try {
    ...
    console.error("[Netherlands RSS] Failed to normalize entry:", err);
    return null;
  }
}
...
/**
 * Returns normalized Netherlands Government articles.
...
export async function getNetherlandsNews(): Promise<NewsArticle[]> {
  ...
  console.log(`[Netherlands RSS] Loaded ${articles.length} articles from feed.`);
  return articles;
}
